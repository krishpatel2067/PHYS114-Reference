...
A PDF stands for \textbf{probability density function}. This class only focuses on the Gaussian, Binomial, and Poisson distributions. The rest are extra information.

\vsp

Remember that probabilities multiply. Thus, the probability of reproducing an entire dataset is

\begin{equation*}
    \prod_{i=1}^{N} \text{PDF}(x_i) = p_1 p_2 \dots p_N\text{.}
\end{equation*}
...
\subsection{Independence, Correlation, and Covariance}
...
\subsection{Stationary PDFs}

The moments in \textbf{nonstationary} PDFs are in terms of time. For \textbf{wide-sense stationary (WSS)} PDFs, only the \emph{first two moments} are stationary, which represents most real-life phenomena. The moments in \textbf{stationary} PDFs are all constant.
...
\begin{enumerate}
    \item The PDF is \textbf{stationary} or \textbf{wide-sense stationary (WSS)}: $\sigma_i \approx \sigma$
    \item Each measurement is \textbf{independent and identically distributed (IID)}.
    \item Use the \textbf{Method of Maximum Liklihood (MML)}, which is the maximum probability of reproducing the data in an "alternate universe".
          \begin{itemize}
          ...
          \end{itemize}
    \item An implicit assumption is the zero-mean process.Assume the mean is zero or subtract off the mean to make calculations much easier.
\end{enumerate}

\subsection{Moment Generating Functions}

\begin{align*}
    n^\text{th} \text{ moment} & = \int_{-\infty}^{\infty} x^n \, \text{PDF}(x) \, \partial x \tag{continuous} \\
                               & = \sum_{i=1}^{\infty} x_i^n \, \text{PDF}(x_i) \tag{discrete}
\end{align*}
...
\begin{itemize}
    ...
    \item Uncertainty for $1^\text{st}$ moment:
          ...
          This indicates diminishing returns for increasing $N$.
...
\begin{equation*}
    \binom{n}{x} = \frac{n!}{x!(n-x)!}\text{,}
\end{equation*}

$n$ is the number of trials, and $x$ is the number of successes.

\begin{itemize}
    \item The $n$ trials must be \textbf{independent}.
...
where $\mu$ is the location parameter and $b$ is the diversity parameter.
...
where $\Gamma$ is the gamma function, $\alpha$ is the shape parameter, and $\beta$ is the scale parameter.
...
\begin{itemize}
    ...
    \item $\sigma_i^2$ is the variance of the $i$-th measurement.
\end{itemize}

$\chi^2$ is a measure of the goodness of fit of a model to the data. A smaller $\chi^2$ indicates a better fit. However, a high error $\sigma_i$ will result in a low $\chi^2$, so a suspciously good chi-square may indicate overly generous error bars. Thus, although the ideal value for chi-square is 0, in practice it should be around $n$ due to the numerator and denominator of the summation being around the same order of magnitude.

\subsubsection{Reduced Chi-Squared}

The reduced chi-square normalizes the chi-square goodness of fit by the number of degrees of freedom. This is useful for comparing fits of different models to the same data.
